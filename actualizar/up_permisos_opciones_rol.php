...
try {
    ...
    $sql = "SELECT
            ...
            FROM
                `seg_rol_permisos`
            WHERE (`id_rol` = $id_rol AND `id_opcion` = $id_opcion)";
    $rs = $cmd->query($sql);
    $existe = $rs->fetch();
    $cmd = null;
...
try {
    ...
    if (empty($existe)) {
        $val = 1;
        $sql = "INSERT INTO `seg_rol_permisos` (`id_rol`, `id_opcion`, `$tpermiso`) VALUES (?, ?, ?)";
        $sql = $cmd->prepare($sql);
        ...
        $val = $estado == 0 ? 1 : 0;
        $sql = "UPDATE `seg_rol_permisos`  SET `$tpermiso` = ? WHERE `id_rol` = ? AND `id_opcion` = ?";
        $sql = $cmd->prepare($sql);
        $sql->bindParam(1, $val);
        $sql->bindParam(2, $id_rol);
