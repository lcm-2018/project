...
$id_det_actfijo = isset($_POST['id_det_actfijo']) ? $_POST['id_det_actfijo'] : exit('Acción no permitida');
...
try {
    ...
    $sql->bindParam(9, $id_det_actfijo, PDO::PARAM_INT);
    if (!($sql->execute())) {
        ...
        if ($sql->rowCount() > 0) {
            ...
            $sql->bindParam(3, $id_det_actfijo, PDO::PARAM_INT);
            $sql->execute();
            if ($sql->rowCount() > 0) {
                $cambio = 1;
            } else {
